...
class tests_list_widget : public QWidget
{
  Q_OBJECT
public:
  tests_list_widget( QWidget* parent = nullptr );

  const QVector< test_series_t >& tests( ) const;
...
signals:
  void test_series_updated( int index );
  void test_series_removed( int index );
  void current_row_changed( int index );

private slots:
  void new_test_series_item( );
  void remove_test_series_item( );
  void edit_test_series_item( );
