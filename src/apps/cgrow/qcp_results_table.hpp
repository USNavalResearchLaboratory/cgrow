...
class qcp_results_table : QCPLayoutGrid
{
  Q_OBJECT

  Q_PROPERTY( QPen border_pen_ READ border_pen WRITE set_border_pen )
  Q_PROPERTY( QBrush brush_ READ brush WRITE set_brush )
  Q_PROPERTY( QFont font READ font WRITE set_font )

public:
  qcp_results_table( QCustomPlot* plot );

  void set_visible( bool on );

  const QPen& border_pen( ) const;
  void        set_border_pen( const QPen pen );
  ...
  double DeltaK_thr( ) const;
  void   set_DeltaK_thr( const double& DeltaK_thr );
