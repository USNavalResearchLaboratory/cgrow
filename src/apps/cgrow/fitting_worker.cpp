                        ...
                        const std::vector<test_data_t> &test_set)
{
    ...
    std::function<void(param_t, param_t, param_t )> update_callback=[this](
            param_t params,
            param_t params_lower,
            param_t params_upper)
    {
        calback_mutex.lock();
    ...
    cg::Hartman_Schijve::fit(
                params_low, params_high,
                test_set_fitting,
                subdivisions, amortization, 0, use_geometric,
                update_callback, progress_report_callback,
                stop_requested_ );
