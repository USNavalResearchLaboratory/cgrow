...
qreal screenScale( QWidget * widget )
{
    ...
    auto screen = QGuiApplication::screenAt( parent->geometry().center() ); // widget->window() is not working as expected here
...
mainWindow::mainWindow(QWidget *parent) : QMainWindow(parent)
{
...

    {
        ...
        connect( tests_list, SIGNAL( current_row_changed(int)), this, SLOT(handle_selected_test_changed( int )));
    }

    {
...

        {
            ...
            eqnLabel->setPixmap( eqnPic.scaled( 240,70, Qt::KeepAspectRatio, Qt::SmoothTransformation ));
...

        {
            ...
            connect( A_min_auto, &QCheckBox::stateChanged, [ this ]( int )
            {
                A_min_auto_set();
            });
            ...
            connect( A_max_auto, &QCheckBox::stateChanged, [ this ]( int )
            {
                A_max_auto_set();
            });
...

    {
        ...
        open_file_action->setStatusTip( tr("Open diagram.") ) ;
    ...
    {
        font_size_spinbox = new decorated_double_spinbox(
                    "",
                    9,
                    0.5, 200,
                    QPixmap("://assets/icons/font_size.png"));
        ...
        connect( font_size_spinbox, SIGNAL(value_changed(double)), this, SLOT(change_font_size(double)));
...

    {
        axes_line_width_spinbox = new decorated_double_spinbox(
                    ...
                    1,
                    0.25, 200,
                    QPixmap("://assets/icons/axes_line_width.png"));
        ...
        connect( axes_line_width_spinbox, SIGNAL(value_changed(double)), this, SLOT(change_axes_line_width(double)));
...

    {
        grid_line_width_spinbox = new decorated_double_spinbox(
                    ...
                    1,
                    0.25, 200,
                    QPixmap("://assets/icons/grid_line_width.png"));
        ...
        connect( grid_line_width_spinbox, SIGNAL(value_changed(double)), this, SLOT(change_grid_lines_width(double)));
    }

    {
        marker_size_spinbox = new decorated_double_spinbox(
                    ...
                    6,
                    0.5, 200,
                    QPixmap("://assets/icons/marker_resize.png"));
        ...
        connect( marker_size_spinbox, SIGNAL(value_changed(double)), this, SLOT(change_marker_size(double)));
    }

    {
        marker_line_width_spinbox = new decorated_double_spinbox(
                    ...
                    1.5,
                    0.25, 200,
                    QPixmap("://assets/icons/marker_line_width.png"));
        ...
        connect( marker_line_width_spinbox, SIGNAL(value_changed(double)), this, SLOT(change_marker_line_width(double)));
    }
    ...
    QWidget *axes_spec_labels = new QWidget;
    {
        ...
        connect( x_spec_label, SIGNAL(textChanged(QString)), this, SLOT(change_x_spec_label(QString)));
        ...
        connect( y_spec_label, SIGNAL(textChanged(QString)), this, SLOT(change_y_spec_label(QString)));
...
void mainWindow::new_file( )
{
    ...
    if( fitted_line_graph )
    {
        plot->removeGraph( fitted_line_graph );
        fitted_line_graph = nullptr;
    }
...
void mainWindow::fit()
{
    ...
    if ( fitted_line_graph != nullptr )
    {
        plot->removeGraph( fitted_line_graph );
    }
    fitted_line_graph = plot->addGraph();
    fitted_line_graph->setName("Computed");

    QPen pen;
    pen.setWidthF( 2.0 );
    pen.setColor( Qt::red );
    fitted_line_graph->setPen(pen);
    ...
    auto params_low  = hs_parameters_t{
            D_min->text().toDouble(),
            p_min->text().toDouble(),
            DeltaK_thr_min->text().toDouble(),
            A_min->text().toDouble() };

    auto params_high = hs_parameters_t{
            D_max->text().toDouble(),
            p_max->text().toDouble(),
...
void mainWindow::handleResults( hs_parameters_t params, hs_parameters_t params_lower, hs_parameters_t params_upper)
{
    computed_ = params;
    ...
    auto generate_hs_curve = []( const auto &params, const auto& R)
    {
        ...
        auto DKs = generate_sequence(
                    params.DeltaK_thr * (1.0 + 1e-6),  // From
